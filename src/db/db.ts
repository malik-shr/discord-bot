...
export class DatabaseService {
  ...
  getWelcomeRole(guild_id: string) {
        return this.db
      .query<WelcomeRole, any>(`SELECT guild_id, role_id FROM welcome_roles WHERE guild_id = ?`)
      .get(guild_id);
  }

  getWelcomeChannel(guild_id: string) {
        return this.db
      .query<WelcomeChannel, any>(`SELECT guild_id, channel_id FROM welcome_channels WHERE guild_id = ?`)
      .get(guild_id);
  }
  ...
  getGuilds(): Guild[] {
    return this.db
      .query<Guild, any>(`SELECT id, name FROM guilds`)
      .all();
  }
}
